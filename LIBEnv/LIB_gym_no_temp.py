...
class LIBPackEnv(gymnasium.Env):
  def __init__(self, model='models/PANmodel.mat', number_of_cells=1, T=25, dt=1, current_ratio=-100, use_priority=True, use_switch=False):
    ...
    self.voltage_idx = 1
    self.temp_idx = 2
    self.prev_action = np.zeros(number_of_cells)
    self.MAX_STEP = 3e3
    self.step_counter = 0
    ...
    if self.use_switch:
      self.action_space = spaces.MultiDiscrete([2]*self.number_of_cells)
    else:
      self.action_space = spaces.Box(low=0, high=np.ones(self.number_of_cells), shape=(number_of_cells,), dtype=np.float32)
  ...
  def step(self, action):
    ...
    self.step_counter += 1
    current = action*self.current_ratio
    coolant_temp = self.T
  ...
  def reset(self, seed=None, options={}):
    ...
    self.step_counter = 0
    self.prev_action = np.zeros(self.number_of_cells)
